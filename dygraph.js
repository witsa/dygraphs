...
Dygraph.prototype.numColumns = function() {
  // [WIT] takes the labels count if rawData is missing.
...
Dygraph.prototype.findClosestPoint = function(domX, domY, forcedSetIdx) {
  ...
  var minDistByX = Infinity;
  var distByX, closestPointByX, closestRowByX;
...
Dygraph.prototype.mouseMove_ = function(event) {
  ...
  // [WIT] process when there are highlightSeries too.
  if ((highlightSeriesOpts || (highlightSeries && highlightSeries.length > 0)) && !this.isSeriesLocked()) {
    var closest;
    
    // [WIT] process when there are no highlightSerie too.
...
Dygraph.prototype.updateSelection_ = function(opt_animFraction) {
  ...
  if (this.attr_('highlightSeriesOpts') || (this.attr_('highlightSeries') && this.attr_('highlightSeries').length > 0)) {
    ...
    if (alpha) {
      ...
      ctx.fillStyle = this.attr_('highlightSeriesBackgroundColor') ?
        this.attr_('highlightSeriesBackgroundColor') : 'rgba(255,255,255,' + alpha + ')';
  ...
  if (this.selPoints_.length > 0) {
    ...
    for (i = 0; i < this.selPoints_.length; i++) {
      ...
      // [WIT] add all the pt in arguments list and not only the pt.idx
...
* @return [low, high]
*/
// [WIT] add seriesName argument for witbar.
Dygraph.prototype.extremeValues_ = function(series, seriesName) {
  ...
  } else if (this.attr_("witBars", seriesName)) { // [WIT] get extreme for a witbars series
    for (j = 0; j < series.length; j++) {
      y = series[j][1][0];
      if (y === null || isNaN(y)) continue;
...
Dygraph.prototype.predraw_ = function() {
  ...
  for (var i = 1; i < this.numColumns(); i++) {
    ...
    // [WIT] add label argument to rollingAverage.
    var labels = this.attr_('labels');
...
// [WIT] witbars point.
Dygraph.witPoints_ = function(points, series) {
  for (var i = 0; i < series.length; ++i) {
    var item = series[i][1];
    var point = points[i];
    point.yval = DygraphLayout.parseFloat_(item[0]);
    point.yMinVal = DygraphLayout.parseFloat_(item[1]);
    point.yMaxVal = DygraphLayout.parseFloat_(item[2]);
    point.yInVal = DygraphLayout.parseFloat_(item[3]);
    point.yOutVal = DygraphLayout.parseFloat_(item[4]);
    point.count = DygraphLayout.parseFloat_(item[5]);
  }
};
...
Dygraph.prototype.gatherDatasets_ = function(rolledSeries, dateWindow) {
  ...
  // [WIT] witBars detection.
  var self = this;
  var witBars =  function (seriesIndex) {
    return self.attr_("witBars", labels[seriesIndex]);
  };
  
  var bars = errorBars || customBars;
  
  // [WIT] isValueNull depends on series type.
  ...
  //  }
  //};
  var isValueNull = function(sample, seriesIndex) {
    if (!bars && !witBars(seriesIndex)) {
      return sample[1] === null;
    } else {
      return customBars ? sample[1][1] === null : 
        (errorBars || witBars(seriesIndex)) ? sample[1][0] === null : false;
    }
  };
  ...
  for (i = num_series; i >= 1; i--) {
    ...
    if (dateWindow) {
      ...
      while (isInvalidValue && correctedFirstIdx > 0) {
        ...
        // [WIT] isValueNull depends on i.
        isInvalidValue = isValueNull(series[correctedFirstIdx], i);
      }
      ...
      while (isInvalidValue && correctedLastIdx < series.length - 1) {
        ...
        // [WIT] isValueNull depends on i.
        isInvalidValue = isValueNull(series[correctedLastIdx], i);
      }
    ...
    // [WIT] extremeValues_ depends on seriesName for witBars.
    var seriesExtremes = this.extremeValues_(series, seriesName);
    ...
    // [WIT] witbars.
    if (witBars(i)) {
      Dygraph.witPoints_(seriesPoints, series);
    }
